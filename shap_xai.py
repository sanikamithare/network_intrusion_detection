# ------------------------------
# Isolation Forest + SHAP (Top 5 Features + Heatmap)
# ------------------------------
...
# 8. Full SHAP Heatmap
# ------------------------------
print("\nGenerating SHAP heatmap for Isolation Forest...")

# Mean |SHAP| values per feature
mean_abs_if = np.mean(np.abs(shap_vals_if), axis=0)

# Convert into DataFrame
shap_if_df = pd.DataFrame(mean_abs_if.reshape(1, -1),
                          columns=X.columns,
                          index=['IsolationForest'])
...
plt.title("SHAP - IsolationForest (mean |value| per feature)")
plt.xticks(rotation=90)
plt.yticks(rotation=0)
plt.tight_layout()

# Save + Show
plt.savefig(os.path.join(OUT_DIR, "shap_if_heatmap.png"))
plt.show()
